...
function formatAnalysis(data: SolutionData & { tags: string[] }) {
  ...
  const remarks = splitByPTag(data.solution)
    ...
    .map((text) => {
      ...
      let result = text.replace(regex, '<b>$1</b>项')
  ...
  if (data.tags?.[0].includes('数量关系')) {
    data.solution = remarks.join('<br>')
    return data
  }
...
export const formatSolution = <T extends SolutionData & {
  tags: string[]
}>(data: T): T => flow(formatEllipsis, formatAnalysis, formatTitle, formatSource)(data) as T
